...
package extension MarkdownRenderer {
    ...
    func declaration(_ fragmentsByLanguage: [SourceLanguage: [DeclarationFragment]]) -> HTMLNode {
        ...
        guard goal == .richness else {
            ...
            let plainTextDeclaration: [HTMLNode] = fragmentsByLanguage.first.map { _, fragments in
                ...
                [code(contents: [.text(fragments.map(\.spelling).joined())])]
            } ?? []
            return pre(contents: plainTextDeclaration)
        }
        
        let declarations: [HTMLNode] = if fragmentsByLanguage.count == 1 {
            ...
            fragmentsByLanguage.map { language, fragments in
                code(attributes: [language.filterAttribute], contents: _declarationTokens(for: fragments, in: language))
            }
        }
        return pre(attributes: [.id("declaration")], contents: declarations)
    }
    ...
    fileprivate func render(_ fragment: DeclarationFormatter.Fragment) -> HTMLNode {
        let text = HTMLNode.text(String(fragment.text))
        ...
        case .keyword, .attribute, .number, .string, .internalParameter:
            ...
            return span(attributes: [.class(fragment.kind.htmlClassName)], contents: [consume text])
        }
    }
}
...
private enum DeclarationFormatter {
    ...
    static func prettyPrintedObjectiveCDeclaration<LinkProvider>(_ fragments: [MarkdownRenderer.DeclarationFragment], using renderer: MarkdownRenderer<LinkProvider>) -> [HTMLNode] {
