...
extension OutOfProcessReferenceResolver {
    ...
    public enum ResponseV2: Codable {
        ...
        ///
        /// - Note: DocC expects the resolved ``LinkDestinationSummary/referenceURL`` to have a "host" component that matches the ``DocumentationContext/Inputs/Identifier`` that your resolver provided in its initial ``identifierAndCapabilities(_:_:)`` handshake message.
        ///   Responding with a ``LinkDestinationSummary/referenceURL`` that doesn't match the resolver's provided ``DocumentationContext/Inputs/Identifier`` is undefined behavior.
