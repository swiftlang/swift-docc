    ...
    mutating func loadAll() throws {
        ...
        let loadGraphAtURL: (URL) -> Void = { [dataLoader, bundle] symbolGraphURL in
            ...
            do {
                ...
                if symbolGraph.symbols.values.mapFirst(where: { SourceLanguage(id: $0.identifier.interfaceLanguage) }) == .objectiveC {
                    ...
                    for (usr, symbol) in symbolGraph.symbols {
                        ...
                        // Clang _doesn't_ use "enum (unnamed)" for the `Symbol/Names/subHeading` so we don't add it here either.
