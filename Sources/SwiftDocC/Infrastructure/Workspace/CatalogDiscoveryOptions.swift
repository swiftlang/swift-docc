...
public struct CatalogDiscoveryOptions {
    ...
    /// Creates new catalog discovery options with the provided documentation input info as Info.plist fallback values.
    ///
    /// - Parameters:
    ///   - fallbackInfo: Fallback documentation input information to use if any discovered catalogs are missing an Info.plist.
    ...
    public init(
        ...
        // Use JSONEncoder to dynamically create the Info.plist fallback dictionary the `CatalogDiscoveryOption`s expect from given `DocumentationContext.Inputs.Info` model.
...
@available(*, deprecated, renamed: "CatalogDiscoveryOptions", message: "Use 'CatalogDiscoveryOptions' instead. This deprecated API will be removed after 6.5 is released.")
