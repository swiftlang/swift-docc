...
extension NavigatorIndex {
    ...
    public struct Identifier: Hashable {
        ...
        func isEquivalentIgnoringLanguage(to other: Identifier) -> Bool {
            return self.bundleIdentifier == other.bundleIdentifier && self.path == other.path && self.fragment == other.fragment
        }
    }
    ...
    open class Builder {
        ...
        package func index(renderNode: ExternalRenderNode, ignoringLanguage: Bool = false) throws {
            ...
            guard
                let objCVariantTrait = renderNode.variants?.flatMap(\.traits).first(where: { trait in
                    switch trait {
        ...
        public func finalize(
            ...
            if groupByLanguage {
                for language in navigatorIndex.availabilityIndex.interfaceLanguages {
                    let languageNode = NavigatorTree.Node(
                        item: NavigatorItem(
                            pageType: PageType.languageGroup.rawValue,
                            languageID: language.mask,
                            title: language.name,
            ...
            if groupByLanguage {
                if !fallouts.isEmpty {
                    ...
                    let otherNode = NavigatorTree.Node(
                        item: NavigatorItem(
                            pageType: PageType.languageGroup.rawValue,
            ...
            if emitJSONRepresentation {
                ...
                do {
                    ...
                    diagnostics.append(
                        error.makeDiagnostic(
                            source: nil, severity: .error, summaryPrefix: "Failed to write render index JSON to '\(jsonNavigatorIndexURL)': "
                        )
                    )
                }
            ...
            if let alreadyDefinedEnvironment = navigatorIndex.environment {
                ...
                do {
                    ...
                    diagnostics.append(
                        error.makeDiagnostic(
                            source: nil, severity: .error, summaryPrefix: "Failed to create navigator index LMDB environment: "
                        )
                    )
