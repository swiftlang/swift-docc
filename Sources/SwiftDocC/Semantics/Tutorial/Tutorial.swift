    ...
    func validate(source: URL?, diagnostics: inout [Diagnostic], featureFlags _: FeatureFlags) -> Bool {
        ...
        sections = sections.filter { section -> Bool in
            ...
            if let previousRange = seenSectionTitles[section.title] {
                ...
                if let source {
                    diagnostic.notes.append(.init(source: source, range: previousRange, message: "First \(TutorialSection.directiveName.singleQuoted) directive with the title '\(section.title)' written here"))
                }
