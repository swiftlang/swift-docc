...
extension Semantic.Analyses {
    ...
    public struct HasExactlyOneOf<Parent: Semantic & DirectiveConvertible, Child1: Semantic & DirectiveConvertible, Child2: Semantic & DirectiveConvertible> {
        ...
        public func analyze(_ directive: BlockDirective, children: some Sequence<any Markup>, source: URL?, for bundle: DocumentationContext.Inputs, diagnostics: inout [Diagnostic]) -> (Child1?, Child2?, remainder: MarkupContainer) {
            let (candidates, remainder) = children.categorize { child -> BlockDirective? in
    ...
    public struct HasExactlyOneImageOrVideoMedia<Parent: Semantic & DirectiveConvertible> {
        ...
        public func analyze(_ directive: BlockDirective, children: some Sequence<any Markup>, source: URL?, for bundle: DocumentationContext.Inputs, diagnostics: inout [Diagnostic]) -> ((any Media)?, remainder: MarkupContainer) {
    ...
    public struct HasExactlyOneMedia<Parent: Semantic & DirectiveConvertible> {
        let severityIfNotFound: DiagnosticSeverity?
        let featureFlags: FeatureFlags
        init(severityIfNotFound: DiagnosticSeverity?, featureFlags: FeatureFlags) {
            self.severityIfNotFound = severityIfNotFound
            self.featureFlags = featureFlags
        }
        ...
        func analyze(_ directive: BlockDirective, children: some Sequence<any Markup>, source: URL?, for bundle: DocumentationContext.Inputs, diagnostics: inout [Diagnostic]) -> ((any Media)?, remainder: MarkupContainer) {
