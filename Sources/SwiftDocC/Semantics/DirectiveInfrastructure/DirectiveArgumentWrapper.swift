...
extension DirectiveArgumentWrapped where Value == Bool {
    @_disfavoredOverload
    init(
        name: _DirectiveArgumentName = .inferredFromPropertyName,
        trueSpelling: String,
        falseSpelling: String,
        required: Bool = false
    ) {
        self = .init(value: nil, name: name, trueSpelling: trueSpelling, falseSpelling: falseSpelling, required: required)
    }
    ...
    init(
        ...
        self = .init(value: wrappedValue, name: name, trueSpelling: trueSpelling, falseSpelling: falseSpelling, required: required)
    }
    
    private init(value: Value?, name: _DirectiveArgumentName, trueSpelling: String, falseSpelling: String, required: Bool) {
        self = .init(
            value: value,
            name: name,
            transform: { _, argument in
                switch argument {
                case trueSpelling:
                    return true
                case falseSpelling:
            ...
            allowedValues: [trueSpelling, falseSpelling],
            required: required
        )
    }
}
