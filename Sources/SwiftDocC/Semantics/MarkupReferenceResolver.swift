...
struct MarkupReferenceResolver: MarkupRewriter {
    ...
    mutating func visitLink(_ link: Link) -> (any Markup)? {
        ...
        guard let resolvedURL = resolve(reference: unresolved, range: link.range, severity: .warning, originalMarkup: link) else {
            return link
        }
        var link = link
    ...
    mutating func resolveAbsoluteSymbolLink(unresolvedDestination: String, elementRange range: SourceRange?, originalMarkup: any AnyLink) -> ResolvedTopicReference? {
        ...
        return resolve(reference: .unresolved(.init(topicURL: url)), range: range, severity: .warning, fromSymbolLink: true, originalMarkup: originalMarkup)
    }
    ...
    mutating func visitSymbolLink(_ symbolLink: SymbolLink) -> (any Markup)? {
        ...
        if let resolved = resolveAbsoluteSymbolLink(unresolvedDestination: destination, elementRange: symbolLink.range, originalMarkup: symbolLink) {
