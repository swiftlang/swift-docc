...
public final class Metadata: Semantic, AutomaticDirectiveConvertible {
  ...
  @available(
    *, deprecated, message: "Do not call directly. Required for 'AutomaticDirectiveConvertible'."
  )
  ...
  func validate(source: URL?, problems: inout [Problem]) -> Bool {
    ...
    if documentationOptions == nil && technologyRoot == nil && displayName == nil
        && pageImages.isEmpty && customMetadata.isEmpty && callToAction == nil && availability.isEmpty
        && pageKind == nil && pageColor == nil && titleHeading == nil && redirects == nil
        && alternateRepresentations.isEmpty
    {
      let diagnostic = Diagnostic(
        ...
        summary:
          "\(Metadata.directiveName.singleQuoted) doesn't configure anything and has no effect"
      )
      ...
      originalMarkup.range.map {
        [
          Solution(
            summary: "Remove this \(Metadata.directiveName.singleQuoted) directive.",
            replacements: [Replacement(range: $0, replacement: "")])
        ]
      } ?? []
    ...
    for pageImages in categorizedPageImages.values {
      ...
      for extraPageImage in pageImages {
        let diagnostic = Diagnostic(
          ...
          summary:
            "Duplicate \(PageImage.directiveName.singleQuoted) directive with \(extraPageImage.purpose.rawValue.singleQuoted) purpose",
          explanation: """
        ...
        let solution = Solution(
          summary:
            "Remove extraneous \(extraPageImage.purpose.rawValue.singleQuoted) \(PageImage.directiveName.singleQuoted) directive",
          replacements: [
    ...
    for duplicateIntroduced in categorizedAvailability.values {
      ...
      for availability in duplicateIntroduced {
        let diagnostic = Diagnostic(
          ...
          summary:
            "Duplicate \(Metadata.Availability.directiveName.singleQuoted) directive with 'introduced' argument",
          explanation: """
        ...
        let solution = Solution(
          summary:
            "Remove extraneous \(Metadata.Availability.directiveName.singleQuoted) directive",
          replacements: [
            Replacement(range: range, replacement: "")
          ]
        )
    ...
    problems.append(
      contentsOf: namesAndRanges.map { (name, range) in
        let diagnostic = Diagnostic(
          ...
          range: range,
          identifier:
            "org.swift.docc.\(Metadata.directiveName).Invalid\(name)InDocumentationComment",
          summary:
            "Invalid use of \(name.singleQuoted) directive in documentation comment; configuration will be ignored",
