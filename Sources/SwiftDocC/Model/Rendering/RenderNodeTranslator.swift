...
public struct RenderNodeTranslator: SemanticVisitor {
    ...
    private func renderAvailabilities(
        ...
        for availability in availabilities {
            let current: PlatformVersion? =
                if let currentPlatforms, let name = PlatformName(metadataPlatform: availability.platform) {
    ...
    public mutating func visitIntro(_ intro: Intro) -> (any RenderTree)? {
        ...
        section.backgroundImage =
            intro.video?.poster.flatMap { createAndRegisterRenderReference(forMedia: $0) }
    ...
    private func firstTutorial(ofTechnology technology: ResolvedTopicReference) -> (reference: ResolvedTopicReference, kind: DocumentationNode.Kind)? {
        guard let volume = (context.children(of: technology, kind: .volume)).first,
            ...
            let firstTutorial = (context.children(of: firstChapter.reference)).first
        else {
            return nil
        }
    ...
    public mutating func visitArticle(_ article: Article) -> (any RenderTree)? {
        ...
        node.topicSectionsVariants =
            VariantCollection<[TaskGroupRenderSection]>(
                ...
                // child kind.
                if (article.topics == nil || article.topics?.taskGroups.isEmpty == true) && article.automaticTaskGroups.isEmpty {
        ...
        if let availabilities = article.metadata?.availability, !availabilities.isEmpty {
            ...
            var directiveAvailabilityByPlatform =
                documentationNode.metadata.map {
    ...
    public mutating func visitSymbol(_ symbol: Symbol) -> (any RenderTree)? {
        ...
        let directiveAvailabilityByPlatform =
            documentationNode.metadata.map {
                renderAvailabilities(from: $0.availability, currentPlatforms: currentPlatforms)
            } ?? .init()
        ...
        node.metadata.platformsVariants =
            VariantCollection<[AvailabilityRenderItem]?>(from: symbol.availabilityVariants) { _, inSourceAvailability in
        ...
        node.metadata.symbolKindVariants =
            VariantCollection<String?>(from: symbol.kindVariants) { _, kindVariants in
        ...
        if shouldEmitSymbolSourceFileURIs {
            node.metadata.sourceFileURIVariants =
        ...
        if let sourceRepository {
            node.metadata.remoteSourceVariants =
                VariantCollection<RenderMetadata.RemoteSource?>(
                    from: symbol.locationVariants
                ) { _, location in
        ...
        node.relationshipSectionsVariants =
            VariantCollection<[RelationshipsRenderSection]>(
                from: documentationNode.availableVariantTraits,
                fallbackDefaultValue: []
            ) { trait in
        ...
        node.defaultImplementationsSectionsVariants =
            VariantCollection<[TaskGroupRenderSection]>(
        ...
        node.deprecationSummaryVariants =
            VariantCollection(
                from: documentationNode.availableVariantTraits,
    ...
    mutating func createAndRegisterRenderReference(forMedia media: ResourceReference?, poster: ResourceReference? = nil, altText: String? = nil, assetContext: DataAsset.Context = .display) -> RenderReferenceIdentifier? {
        guard let oldMedia = media,
            let mediaIdentifier = context.identifier(forAssetName: oldMedia.path, in: identifier)
        else {
            return nil
        }
        ...
        guard
            let resolvedAssets = renderContext?.store.content(forAssetNamed: media.path, bundleID: identifier.bundleID)
