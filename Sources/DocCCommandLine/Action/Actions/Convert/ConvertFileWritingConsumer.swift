...
struct ConvertFileWritingConsumer: ConvertOutputConsumer, ExternalNodeConsumer, ConvertOutputMarkdownConsumer {
    ...
    func consume(assetsInInputs inputs: DocumentationContext.Inputs) throws {
        ...
        assert(inputsID.rawValue == self.assetPrefixComponent, "Unexpectedly encoding assets for a bundle other than the one this output consumer was created for.")
        
        // Create images directory if needed.
        let imagesDirectory = targetFolder
            .appendingPathComponent("images", isDirectory: true)
            .appendingPathComponent(inputsID.rawValue, isDirectory: true)
        ...
        for imageAsset in context.registeredImageAssets(for: inputsID) {
        ...
        for videoAsset in context.registeredVideoAssets(for: inputsID) {
        ...
        if let customFavicon = inputs.customFavicon {
