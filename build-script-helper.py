...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        help='Log the executed commands.')
    ...
    parser.add_argument(
        ...
        help='Path to an Xcode workspace to create a unified build of '
             'Swift-DocC with other projects.')
    parser.add_argument(
        '--toolchain', required=True,
        help='The toolchain to use when building this package.')
    parser.add_argument(
        '--update', action='store_true',
        help='Update all SwiftPM dependencies.')
    parser.add_argument(
        '--no-local-deps', action='store_true',
        help='Use normal remote dependencies when building.')
    parser.add_argument(
        'build_actions',
        help='Extra actions to perform. Can be any number of the following',
        choices=['all', 'build', 'test', 'generate-xcodeproj', 'install'],
        nargs="*", default=['build'])
    parser.add_argument(
        '--install-dir', default=None,
        help='The location to install the docc executable to.')
    parser.add_argument(
        '--copy-doccrender-from', default=None,
        help='The location to copy an existing Swift-DocC-Render '
             'template from.')
    parser.add_argument(
        '--copy-doccrender-to', default=None,
        help='The location to install an existing Swift-DocC-Render '
             'template to.')
    parser.add_argument(
        "--cross-compile-hosts", dest="cross_compile_hosts",
...
def run(args):
    ...
    # The test action creates its own build. No need to build if we are just
    # testing.
...
def update_swiftpm_dependencies(package_path, swift_exec, build_path, env,
                                verbose):
    args = [
        swift_exec, 'package',
...
def get_swiftpm_options(action, args):
    ...
    # Pass the verbose flag for "install" builds to get more information to
    # investigate a CI build failure. (rdar://85912344)
    ...
    if build_os.startswith('macosx'):
        swiftpm_args += [
            # Relative library rpath for swift; will only be used when
            # /usr/lib/swift is not available.
            '-Xlinker', '-rpath',
            '-Xlinker', '@executable_path/../lib/swift/macosx',
        ]
    elif build_os.startswith('freebsd'):
        # FreeBSD triples include the OS version number.
        # The swift runtime library install location does not, resulting in
        # a docc that fails to launch.
        swiftpm_args += [
            # Library rpath for swift, dispatch, Foundation, etc. when
            # installing.
            '-Xlinker', '-rpath',
            '-Xlinker', '$ORIGIN/../lib/swift/freebsd',
        ]
        if action == 'install':
        ...
        swiftpm_args += [
            '-Xlinker', '-rpath',
            '-Xlinker', '$ORIGIN/../lib/swift/openbsd',
        ...
        swiftpm_args += [
            ...
            # installing.
            '-Xlinker', '-rpath',
            '-Xlinker', '$ORIGIN/../lib/swift/' + build_os,
        ]
        if action == 'install':
            swiftpm_args += ['--disable-local-rpath']
    ...
    if cross_compile_hosts:
        if (build_os.startswith('macosx')
                and cross_compile_hosts.startswith('macosx-')):
    ...
    if action == 'install' or action == 'show-bin-path':
        # When tests are run on the host machine, `docc` is located in the
        # build directory; to find its linked libraries (Swift runtime
        # dependencies), `docc` needs to link against the host machine's
        # toolchain libraries. When installing docc on the target machine,
        # the `docc` executable will be located in the toolchain, so it
        # should find its linked libraries at a relative path.
...
def invoke_swift(action, products, env, args, swiftpm_args):
    # Until rdar://53881101 is implemented, we cannot request a build of
    # multiple targets simultaneously. For now, just build one product after
    # the other.
    for product in products:
        invoke_swift_single_product(action, product, env, args, swiftpm_args)


def get_call_to_invoke_swift_single_product(action, product, args,
                                            swiftpm_args):
...
def invoke_swift_single_product(action, product, env, args, swiftpm_args):
    ...
    # Tell Swift-DocC that we are building in a build-script environment so
    # that it does not need to be rebuilt if it has already been built before.
...
def install(args, env):
    ...
    create_intermediate_directories(
        os.path.dirname(docc_install_dir), verbose=verbose)
    check_and_sync(
        file_path=docc_path,
    ...
    # Install features.json relative to the docc executable at
    # "../../share/docc/features.json"
    ...
    create_intermediate_directories(
        os.path.dirname(features_install_path), verbose=verbose)
    check_and_sync(
    ...
    if copy_render_from is not None:
        if copy_render_to is None:
            fatal_error(
                "Missing required '--copy-doccrender-to' argument since "
                "'--copy-doccrender-from' was passed.")
...
def get_build_target(args):
    """Returns the target-triple of the current machine or for cross-
    compilation."""
    # Adapted from
    # https://github.com/swiftlang/swift-package-manager/blob/fde9916d/Utilities/bootstrap#L296
    command = [args.swift_exec, '-print-target-info']
    target_info_json = subprocess.check_output(
        command, stderr=subprocess.PIPE, universal_newlines=True).strip()
...
def create_intermediate_directories(dir_path, verbose):
    ...
    print("-- note: creating intermediate directories %s: %s"
          % (dir_path, " ".join(cmd)))
    result = check_call(cmd, verbose=verbose)
    if result != 0:
        fatal_error(
            "creating intermediate directories failed with exit status %d"
            % (result,))


def check_and_sync(file_path, install_path, verbose):
    cmd = ["rsync", "-a", file_path, install_path]
    print("-- note: installing %s: %s"
          % (os.path.basename(file_path), " ".join(cmd)))
    result = check_call(cmd, verbose=verbose)
    if result != 0:
...
def check_call(cmd, verbose, env=os.environ, **kwargs):
    ...
    return subprocess.check_call(
        cmd, env=env, stderr=subprocess.STDOUT, **kwargs)
