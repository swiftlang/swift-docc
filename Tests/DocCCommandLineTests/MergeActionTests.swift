...
class MergeActionTests: XCTestCase {
    ...
    func testMergingArchivesWithPageImages() async throws {
        ...
        func convertCatalog(named name: String, file: StaticString = #filePath, line: UInt = #line) async throws -> URL {
            ...
            let outputConsumer = ConvertFileWritingConsumer(targetFolder: outputPath, catalogRootFolder: catalogDir, fileManager: fileSystem, outputFileManager: fileSystem, context: context, indexer: indexer, transformForStaticHostingIndexHTML: nil, bundleID: inputs.id)
