...
struct TestFileSystemTests {
    ...
    func dumpSubpath() throws {
        ...
        #expect(
            fileSystem.dump(subHierarchyFrom: "/main") == """
                main/
                ╰─ nested/
                   ├─ myfile1.txt
                   ╰─ myfile2.txt
        ...
        #expect(
            fileSystem.dump(subHierarchyFrom: "/main/nested") == """
                nested/
                ├─ myfile1.txt
                ╰─ myfile2.txt
                """)
    ...
    func copyFolders() throws {
        ...
        #expect(
            filesIterator.prefix(2).map(\.path).sorted() == [
                // Shallow files first
                "/copy/myfile1.txt",
                "/copy/myfile2.txt",
            ])
        #expect(
            filesIterator.dropFirst(2).map(\.path).sorted() == [
                // Deeper files after
    ...
    func createFolders() throws {
        ...
        #expect(
            fileSystem.recursiveFiles(startingPoint: URL(fileURLWithPath: "/")).map(\.lastPathComponent).sorted() == [
