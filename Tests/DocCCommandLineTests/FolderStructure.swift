...
import XCTest
public import DocCTestUtilities
...
extension Folder: AssertableFile {
    func __assertExist(at location: URL, fileManager: any FileManagerProtocol, file: StaticString = #filePath, line: UInt = #line) {
        ...
        XCTAssert(
            fileManager.fileExists(atPath: location.path, isDirectory: &isFolder),
...
extension InfoPlist: AssertableFile {
    func __assertExist(at location: URL, fileManager: any FileManagerProtocol, file: StaticString, line: UInt) {
        ...
        do {
            ...
            XCTAssert(
                displayName == content.displayName && identifier == content.identifier,
...
extension TextFile: AssertableFile {
    func __assertExist(at location: URL, fileManager: any FileManagerProtocol, file: StaticString, line: UInt) {
        XCTAssert(
            ...
            "File '\(name)' should exist at '\(location.path)'", file: (file), line: line)
        XCTAssertEqual(
            fileManager.contents(atPath: location.path).map({ String(data: $0, encoding: .utf8) }), utf8Content,
...
extension JSONFile: AssertableFile {
    func __assertExist(at location: URL, fileManager: any FileManagerProtocol, file: StaticString, line: UInt) {
        ...
        guard let fileData = fileManager.contents(atPath: location.path),
            let other = try? JSONDecoder().decode(Content.self, from: fileData)
        else {
...
extension CopyOfFile: AssertableFile {
    func __assertExist(at location: URL, fileManager: any FileManagerProtocol, file: StaticString, line: UInt) {
        ...
        XCTAssert(
            fileManager.contentsEqual(atPath: original.path, andPath: location.path),
