...
class ConvertActionTests: XCTestCase {
    ...
    func testOutputFolderIsNotRemovedWhenThereAreErrors() async throws {
        let tutorialsFile = TextFile(
            name: "TechnologyX.tutorial",
            utf8Content: """
    ...
    func testDownloadMetadataIsWrittenToOutputFolder() async throws {
        ...
        XCTAssert(
            resultAssets.downloads.contains(where: {
    ...
    func testConvertWithCustomTemplates() async throws {
        ...
        let footer = TextFile(
            ...
            utf8Content: """
                ...
                """)
        let bundle = Folder(
            name: "TestConvertWithCustomTemplates.docc",
            content: [
                info,
                header,
                footer,
    ...
    func testConvertWithCustomTemplatesForStaticHosting() async throws {
        ...
        let technologyPage = TextFile(
            name: "TestConvertWithCustomTemplatesForStaticHosting.md",
            utf8Content: """
                ...
                """)
        let bundle = Folder(
            name: "TestConvertWithCustomTemplatesForStaticHosting.docc",
            content: [
                info,
                header,
                footer,
    ...
    func testConvertWithThemeSettings() async throws {
        ...
        let template = Folder(name: "template", content: [index])
        let bundle = Folder(
            name: "TestConvertWithThemeSettings.docc",
            content: [
                info,
                themeSettings,
            ])
    ...
    func testWarnsWhenTutorialsTableOfContentsPageIsMissing() async throws {
        ...
        XCTAssert(
            onlyTutorialArticleDiagnostics.contains(where: {
        ...
        let tutorialTableOfContentDiagnostics = try await diagnosticsFromConverting([
            InfoPlist(displayName: "TestBundle", identifier: "com.test.example"),
            TextFile(
                name: "table-of-contents.tutorial",
                utf8Content: """
                    """
            ),
            TextFile(
        ...
        XCTAssert(
            tutorialTableOfContentDiagnostics.contains(where: {
                $0.identifier == "MissingTableOfContentsPage"
            }))
        ...
        XCTAssert(
            incompleteTutorialFile.contains(where: {
    ...
    func testWrittenDiagnosticsAfterConvert() async throws {
        ...
        XCTAssertEqual(
            diagnosticFileContent.diagnostics.map(\.summary).sorted(),
            [
    ...
    func testEncodedImagePaths() async throws {
        ...
        XCTAssertEqual(
            fileSystem.dump(subHierarchyFrom: targetURL.path),
            """
            Output.doccarchive/
            ├─ data/
            │  ╰─ documentation/
        ...
        XCTAssertEqual(
            imageReference.asset.variants.values.map(\.absoluteString).sorted(),
            [
