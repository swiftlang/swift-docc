...
class HasExactlyOneTests: XCTestCase {
    func testEmpty() async throws {
        ...
        directive.map { directive in
            ...
            let (match, remainder) = Semantic.Analyses.HasExactlyOne<TestParent, TestChild>(severityIfNotFound: .error).analyze(directive, children: directive.children, source: nil, for: inputs, problems: &problems)
            XCTAssertNil(match)
            ...
            problems.first.map { problem in
                ...
                XCTAssertEqual(
                    DiagnosticConsoleWriter.formattedDescription(for: problem.diagnostic, options: .formatConsoleOutputForTools),
                    """
