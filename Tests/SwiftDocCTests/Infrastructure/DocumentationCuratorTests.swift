...
class DocumentationCuratorTests: XCTestCase {
    ...
    func testCrawlDiagnostics() throws {
        let (tempCatalogURL, bundle, context) = try testBundleAndContext(copying: "DoNotUseInNewTests") { url in
    ...
    func testLinkResolving() throws {
        let (sourceRoot, bundle, context) = try testBundleAndContext(named: "DoNotUseInNewTests")
    ...
    func testGroupLinkValidation() throws {
        let (_, bundle, context) = try testBundleAndContext(copying: "DoNotUseInNewTests", excludingPaths: []) { root in
