...
class DocumentationCuratorTests: XCTestCase {
    ...
    func testCrawl() async throws {
        ...
        var crawler = DocumentationCurator(in: context, inputs: inputs)
    ...
    func testCrawlDiagnostics() async throws {
        let (tempCatalogURL, inputs, context) = try await testBundleAndContext(copying: "LegacyBundle_DoNotUseInNewTests") { url in
    ...
    func testModuleUnderTechnologyRoot() async throws {
        let (_, inputs, context) = try await testBundleAndContext(copying: "SourceLocations") { url in
            try """
        ...
        let crawler = DocumentationCurator(in: context, inputs: inputs)
    ...
    func testCuratorDoesNotRelateNodesWhenArticleLinksContainExtraPathComponents() async throws {
        let (inputs, context) = try await loadBundle(catalog:
        ...
        let renderer = DocumentationNodeConverter(inputs: inputs, context: context)
    ...
    func testLinkResolving() async throws {
        let (sourceRoot, inputs, context) = try await testBundleAndContext(named: "LegacyBundle_DoNotUseInNewTests")
    ...
    func testGroupLinkValidation() async throws {
        let (_, inputs, context) = try await testBundleAndContext(copying: "LegacyBundle_DoNotUseInNewTests", excludingPaths: []) { root in
    ...
    func testMixedManualAndAutomaticCuration() async throws {
        ...
        let reference = ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/TestBed/TopClass/NestedEnum/SecondLevelNesting", sourceLanguage: .swift)
        ...
        let nameReference = ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/TestBed/TopClass/name", sourceLanguage: .swift)
        ...
        let ageReference = ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/TestBed/TopClass/age", sourceLanguage: .swift)
