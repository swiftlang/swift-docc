...
class TopicGraphTests: XCTestCase {
    ...
    func testNodeWithReference() {
        XCTAssertEqual(
            TestGraphs.withOneNode.nodeWithReference(TestGraphs.testNodeWithTitle("A").reference),
            TestGraphs.testNodeWithTitle("A"))

        XCTAssertEqual(
            TestGraphs.withOneEdge.nodeWithReference(TestGraphs.testNodeWithTitle("A").reference),
            TestGraphs.testNodeWithTitle("A"))

        XCTAssertEqual(
            TestGraphs.withOneEdge.nodeWithReference(TestGraphs.testNodeWithTitle("B").reference),
    ...
    func testEveryEdgeSourceHasNode() {
        for graph in [TestGraphs.complex, TestGraphs.withCycle, TestGraphs.withOneEdge, TestGraphs.withOneNode] {
            ...
            XCTAssert(
                missingReverseEdgeReferences.isEmpty,
                """
        ...
        for graph in [TestGraphs.complex, TestGraphs.withCycle, TestGraphs.withOneEdge, TestGraphs.withOneNode] {
            ...
            for (source, targets) in graph.reverseEdges {
                for target in targets {
                    ...
                    XCTAssert(
                        graph.edges[target]?.contains(source) ?? false,
