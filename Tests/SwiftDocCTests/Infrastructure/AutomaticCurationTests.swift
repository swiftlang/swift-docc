...
class AutomaticCurationTests: XCTestCase {
    ...
    func testAutomaticTopicsGenerationForSameModuleTypes() async throws {
        for kind in availableNonExtensionSymbolKinds {
            ...
            try assertRenderedPage(atPath: "/documentation/ModuleName/SomeClass", containsAutomaticTopicSectionFor: kind, context: context, inputs: inputs)
        }
    }
    ...
    func testAutomaticTopicsGenerationForExtensionSymbols() async throws {
        ...
        for kind in availableExtensionSymbolKinds where kind != .extendedModule {
            ...
            try assertRenderedPage(atPath: "/documentation/ModuleName", containsAutomaticTopicSectionFor: .extendedModule, context: context, inputs: inputs)
            try assertRenderedPage(atPath: "/documentation/ModuleName/ExtendedModule", containsAutomaticTopicSectionFor: kind, context: context, inputs: inputs)
        }
    }
    ...
    private func assertRenderedPage(
        ...
        let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: path, sourceLanguage: .swift))
    ...
    func testSeeAlsoSectionForAutomaticallyCuratedTopics() async throws {
        ...
        do {
            let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/SideKit/SideClassFour", sourceLanguage: .swift))
            var translator = RenderNodeTranslator(context: context, inputs: inputs, identifier: node.reference)
            let renderNode = translator.visit(node.semantic as! Symbol) as! RenderNode
        ...
        // The second topic section
        do {
            let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/SideKit/SideClassSix", sourceLanguage: .swift))
            var translator = RenderNodeTranslator(context: context, inputs: inputs, identifier: node.reference)
            let renderNode = translator.visit(node.semantic as! Symbol) as! RenderNode
        ...
        do {
            let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/SideKit/SideClassEight", sourceLanguage: .swift))
            var translator = RenderNodeTranslator(context: context, inputs: inputs, identifier: node.reference)
            let renderNode = translator.visit(node.semantic as! Symbol) as! RenderNode
        ...
        do {
            let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/SideKit/SideClassNine", sourceLanguage: .swift))
            var translator = RenderNodeTranslator(context: context, inputs: inputs, identifier: node.reference)
            let renderNode = translator.visit(node.semantic as! Symbol) as! RenderNode
        ...
        do {
            let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/SideKit/SideClassTen", sourceLanguage: .swift))
            var translator = RenderNodeTranslator(context: context, inputs: inputs, identifier: node.reference)
            let renderNode = translator.visit(node.semantic as! Symbol) as! RenderNode
    ...
    func testTopLevelSymbolsAreNotAutomaticallyCuratedIfManuallyCuratedElsewhere() async throws {
        ...
        do {
            ...
            let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/TestBed", sourceLanguage: .swift))
            var translator = RenderNodeTranslator(context: context, inputs: inputs, identifier: node.reference)
            let renderNode = translator.visit(node.semantic as! Symbol) as! RenderNode
        ...
        do {
            ...
            let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/TestBed/A", sourceLanguage: .swift))
            var translator = RenderNodeTranslator(context: context, inputs: inputs, identifier: node.reference)
            let renderNode = translator.visit(node.semantic as! Symbol) as! RenderNode
    ...
    func testAutomaticCurationDropsOverloadGroupWhenOverloadsAreCurated() async throws {
        ...
        let (_, inputs, context) = try await testBundleAndContext(copying: "OverloadedSymbols") { url in
            try """
            # ``OverloadedProtocol``
        ...
        var translator = RenderNodeTranslator(context: context, inputs: inputs, identifier: protocolDocumentationNode.reference)
        let renderNode = translator.visit(symbol) as! RenderNode
    ...
    func testAutomaticallyCuratedSymbolTopicsAreMergedWithManuallyCuratedTopics() async throws {
         for kind in availableNonExtensionSymbolKinds {
             ...
             let (_, inputs, context) = try await loadBundle(from: catalogURL)

             let node = try context.entity(with: ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/ModuleName/SomeClass", sourceLanguage: .swift))
