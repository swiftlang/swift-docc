...
class VersionTripletTest: XCTestCase {
    ...
    func testInitializeFromSemanticVersion() {
        let semanticVersion = SemanticVersion(major: 1, minor: 2, patch: 3)
        let version = VersionTriplet(semanticVersion)
        XCTAssertEqual(version, VersionTriplet(1, 2, 3))
    }

    func testInitializeFromSymbolKitSemanticVersion() {
        let semanticVersion = SymbolGraph.SemanticVersion(major: 1, minor: 2, patch: 3)
        let version = VersionTriplet(semanticVersion)
        XCTAssertEqual(version, VersionTriplet(1, 2, 3))
    }
