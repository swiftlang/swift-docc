...
class NodeTagsTests: XCTestCase {
    func testSPIMetadata() async throws {
        ...
        let (_, inputs, context) = try await loadBundle(from: tempURL)
        
        // Verify that `Test` is marked as SPI.
        let reference = ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/Minimal_docs/Test", sourceLanguage: .swift)
        ...
        let moduleReference = ResolvedTopicReference(bundleID: inputs.id, path: "/documentation/Minimal_docs", sourceLanguage: .swift)
        ...
        // Verify the render node contains the SPI tag.
        var moduleTranslator = RenderNodeTranslator(context: context, inputs: inputs, identifier: node.reference)
