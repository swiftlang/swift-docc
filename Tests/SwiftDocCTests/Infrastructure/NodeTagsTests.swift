...
class NodeTagsTests: XCTestCase {
    func testSPIMetadata() throws {
        ...
        let catalogFolder = Folder(name: "unit-tests.docc", content: [
        ...
        try catalogFolder.write(to: tempURL)
        
        let (_, catalog, context) = try loadCatalog(from: tempURL)
        
        // Verify that `Test` is marked as SPI.
        let reference = ResolvedTopicReference(catalogIdentifier: catalog.identifier, path: "/documentation/Minimal_docs/Test", sourceLanguage: .swift)
        ...
        // Verify the render node contains the SPI tag.
        var moduleTranslator = RenderNodeTranslator(context: context, catalog: catalog, identifier: node.reference, source: nil)
