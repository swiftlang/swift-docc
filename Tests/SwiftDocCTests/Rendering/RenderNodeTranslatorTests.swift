...
class RenderNodeTranslatorTests: XCTestCase {
    private func findDiscussion(forSymbolPath: String, configureBundle: ((URL) throws -> Void)? = nil) throws -> ContentRenderSection? {
        ...
        let node = try context.entity(with: ResolvedTopicReference(id: bundle.id, path: forSymbolPath, sourceLanguage: .swift))
    ...
    func testSectionAbstracts() throws {
        ...
        let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/Test-Bundle/article", sourceLanguage: .swift)
    ...
    func testEmptyTaskGroupsNotRendered() throws {
        ...
        let reference = ResolvedTopicReference(id: "org.swift.docc.example", path: "/documentation/Test-Bundle/taskgroups", fragment: nil, sourceLanguage: .swift)
    ...
    func testAutomaticTaskGroupsOrderingInSymbols() throws {
        ...
        let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/SideKit/SideClass", sourceLanguage: .swift)
        var translator = RenderNodeTranslator(context: context, bundle: bundle, identifier: reference)
    ...
    func testOrderingOfAutomaticGroupsInDefiningProtocol() throws {
        ...
        do {
            let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/SideKit/SideClass/Element", sourceLanguage: .swift)
            var translator = RenderNodeTranslator(context: context, bundle: bundle, identifier: reference)
            let node = try XCTUnwrap(try? context.entity(with: reference))
        ...
        do {
            let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/SideKit/SideClass/Element/Protocol-Implementations", sourceLanguage: .swift)
            var translator = RenderNodeTranslator(context: context, bundle: bundle, identifier: reference)
            let node = try XCTUnwrap(try? context.entity(with: reference))
    ...
    func testAutomaticImplementationsWithExtraDots() throws {
        ...
        let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/FancyProtocol/SomeClass", sourceLanguage: .swift)
        var translator = RenderNodeTranslator(context: context, bundle: bundle, identifier: reference)
        let node = try context.entity(with: reference)
        let symbol = try XCTUnwrap(node.semantic as? Symbol)
        let renderNode = try XCTUnwrap(translator.visitSymbol(symbol) as? RenderNode)
    ...
    func testAutomaticTaskGroupTopicsAreSorted() throws {
        ...
        let structReference = ResolvedTopicReference(id: bundle.id, path: "/documentation/DefaultImplementations/Foo", sourceLanguage: .swift)
    ...
    func testLinkInAbstract() throws {
        do {
            ...
            let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/SideKit", sourceLanguage: .swift)
            let node = try context.entity(with: reference)
    ...
    func testSnippetToCodeListing() throws {
        ...
        let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/Snippets/Snippets", sourceLanguage: .swift)
    ...
    func testSnippetSliceTrimsIndentation() throws {
        let (bundle, context) = try testBundleAndContext(named: "Snippets")
        let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/Snippets/SliceIndentation", sourceLanguage: .swift)
        let article = try XCTUnwrap(context.entity(with: reference).semantic as? Article)
        var translator = RenderNodeTranslator(context: context, bundle: bundle, identifier: reference)
        let renderNode = try XCTUnwrap(translator.visitArticle(article) as? RenderNode)
        let discussion = try XCTUnwrap(renderNode.primaryContentSections.first(where: { $0.kind == .content }) as? ContentRenderSection)
    ...
    func testExpectedRoleHeadingIsAssigned() throws {
        ...
        func renderNodeArticleFromReferencePath(
            ...
            let reference = ResolvedTopicReference(id: bundle.id, path: referencePath, sourceLanguage: .swift)
