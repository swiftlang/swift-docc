...
class RenderNodeTranslatorTests: XCTestCase {
    private func findDiscussion(forSymbolPath: String, configureBundle: ((URL) throws -> Void)? = nil) throws -> ContentRenderSection? {
        let (_, bundle, context) = try testBundleAndContext(copying: "DoNotUseInNewTests", configureBundle: configureBundle)
    ...
    func testAutomaticTaskGroupsOrderingInSymbols() throws {
        let (_, bundle, context) = try testBundleAndContext(copying: "DoNotUseInNewTests", excludingPaths: [], externalResolvers: [:], externalSymbolResolver: nil, configureBundle: { url in
            try """
            # ``SideKit/SideClass``
            SideClass abstract
            ## Topics
    ...
    func testAutomaticImplementationsWithExtraDots() throws {
        ...
        let (_, bundle, context) = try testBundleAndContext(copying: "DoNotUseInNewTests", excludingPaths: []) { url in
