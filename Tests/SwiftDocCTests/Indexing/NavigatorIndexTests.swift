...
"""
    ...
    func testNavigatorIndexCapturesBetaStatus() throws {
        ...
        let (_, bundle, context) = try testBundleAndContext(named: "AvailabilityBetaBundle", configuration: configuration)
        let renderContext = RenderContext(documentationContext: context, bundle: bundle)
        let converter = DocumentationContextConverter(bundle: bundle, context: context, renderContext: renderContext)
        let targetURL = try createTemporaryDirectory()
        let builder = NavigatorIndex.Builder(outputURL: targetURL, bundleIdentifier: bundle.id.rawValue, sortRootChildrenByName: true)
