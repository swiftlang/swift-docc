...
class RenderNodeDiffingBundleTests: XCTestCase {
    ...
    func testDiffSymbolFromBundleWithDiscussionSectionRemoved() async throws {
        ...
        let modification = { (url: URL) in
            ...
            let text = try String(contentsOf: symbolURL).replacingOccurrences(
                of: """
                    ## Discussion
        ...
        let differences = try await getDiffsFromModifiedDocument(
    ...
    func testDiffArticleFromBundleWithTopicSectionAdded() async throws {
        ...
        let modification = { (url: URL) in
            ...
            let text = try String(contentsOf: articleURL).replacingOccurrences(
                of: "## Topics",
                with: """
                    ## Topics
        ...
        let expectedDiff = JSONPatchOperation.add(
            pointer: JSONPointer(pathComponents: ["topicSections", "0"]),
            value: AnyCodable(
    ...
    func testDiffArticleFromBundleWithSeeAlsoSectionRemoved() async throws {
        ...
        let expectedReferenceDiff = JSONPatchOperation.remove(
            pointer: JSONPointer(pathComponents: [
                "references",
    ...
    func testDiffSymbolFromBundleWithAbstractUpdated() async throws {
        ...
        let expectedAbstractDiff = JSONPatchOperation.add(
            pointer: JSONPointer(pathComponents: ["abstract", "0"]),
    ...
    func testDiffSymbolFromBundleWithDeprecationAdded() async throws {
        ...
        let expectedDeprecationDiff = JSONPatchOperation.add(
            pointer: JSONPointer(pathComponents: ["deprecationSummary"]),
            value: AnyCodable([
        ...
        let expectedReferenceDiff = JSONPatchOperation.replace(
            pointer: JSONPointer(pathComponents: [
                "references",
    ...
    func testDiffSymbolFromBundleWithDisplayNameDirectiveAdded() async throws {
        ...
        let expectedTitleDiff = JSONPatchOperation.replace(
            pointer: JSONPointer(pathComponents: ["metadata", "title"]),
        ...
        let expectedModuleDiff = JSONPatchOperation.add(
            pointer: JSONPointer(pathComponents: ["metadata", "modules", "0"]),
    ...
    func testDiffArticleFromBundleWithDownloadDirectiveAdded() async throws {
        ...
        let expectedRoleHeadingDiff = JSONPatchOperation.replace(
            pointer: JSONPointer(pathComponents: ["metadata", "roleHeading"]),
        ...
        let expectedRoleDiff = JSONPatchOperation.replace(
            pointer: JSONPointer(pathComponents: ["metadata", "role"]),
    ...
    func getDiffsFromModifiedDocument(
        bundleName: String,
        ...
        let nodeOriginal = try contextOriginal.entity(
        ...
        let nodeModified = try contextModified.entity(
            with: ResolvedTopicReference(
                bundleID: bundleID,
                path: topicReferencePath,
                sourceLanguage: .swift))
    ...
    func assertDifferences<Value: Equatable>(
        ...
        guard let foundDiff = differences.first(where: { $0.pointer == expectedDiff.pointer && $0.operation == expectedDiff.operation }) else {
