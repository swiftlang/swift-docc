...
class PropertyListPossibleValuesSectionTests: XCTestCase {
    ...
    func testAbsenceOfPossibleValues() throws {
        ...
        let node = try context.entity(with: ResolvedTopicReference(bundleIdentifier: bundle.id.rawValue, path: "/documentation/DictionaryData/Artist", sourceLanguage: .swift))
    ...
    func testUndocumentedPossibleValues() throws {
        let (_, bundle, context) = try testBundleAndContext(copying: "DictionaryData")
        let node = try context.entity(with: ResolvedTopicReference(bundleIdentifier: bundle.id.rawValue, path: "/documentation/DictionaryData/Month", sourceLanguage: .swift))
        let converter = DocumentationNodeConverter(bundle: bundle, context: context)
