...
class ParametersAndReturnValidatorTests: XCTestCase {
    ...
    func testFiltersParameters() throws {
        ...
        do {
            let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/ErrorParameters/MyClassInObjectiveC/doSomething(with:)", sourceLanguage: .swift)
        ...
        do {
            let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/ErrorParameters/MyClassInObjectiveC/returnSomething()", sourceLanguage: .swift)
            let node = try context.entity(with: reference)
            let symbol = try XCTUnwrap(node.semantic as? Symbol)
        ...
        do {
            let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/ErrorParameters/MyClassInSwift/doSomething(with:)", sourceLanguage: .swift)
            let node = try context.entity(with: reference)
            let symbol = try XCTUnwrap(node.semantic as? Symbol)
        ...
        do {
            let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/ErrorParameters/MyClassInSwift/returnSomething()", sourceLanguage: .swift)
            let node = try context.entity(with: reference)
            let symbol = try XCTUnwrap(node.semantic as? Symbol)
    ...
    func testFunctionWithOnlyErrorParameter() throws {
        ...
        let reference = ResolvedTopicReference(id: bundle.id, path: "/documentation/ModuleName/functionName(...)", sourceLanguage: .swift)
        let node = try context.entity(with: reference)
        let symbol = try XCTUnwrap(node.semantic as? Symbol)
