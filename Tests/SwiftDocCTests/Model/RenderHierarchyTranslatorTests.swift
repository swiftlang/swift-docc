...
class RenderHierarchyTranslatorTests: XCTestCase {
    func test() async throws {
        ...
        let technologyReference = ResolvedTopicReference(bundleID: inputs.id, path: "/tutorials/TestOverview", sourceLanguage: .swift)
        
        var translator = RenderHierarchyTranslator(context: context, inputs: inputs)
    ...
    func testLanguageSpecificHierarchies() async throws {
        let (inputs, context) = try await testBundleAndContext(named: "GeometricalShapes")
