...
struct AvailabilityTests {
        ...
        let catalog = Folder(name: "unit-test.docc") {
            ...
            JSONFile(
                ...
                content: makeSymbolGraph(
                    moduleName: "ModuleName", platform: .init(operatingSystem: .init(name: "ios"), environment: nil),
                    symbols: [
                        makeSymbol(
    ...
    func platformSpecificSymbolWithoutInSourceAvailabilityDoesNotDisplayDefaultAvailabilityForOtherPlatforms() async throws {
        let catalog = Folder(name: "unit-test.docc") {
            JSONFile(
                name: "ModuleName-ios.symbols.json",
                content: makeSymbolGraph(
                    moduleName: "ModuleName", platform: .init(operatingSystem: .init(name: "tvos"), environment: nil),
                    symbols: [
                        makeSymbol(id: "first-symbol-id", kind: .class, pathComponents: ["First"])  // No in-source availability attributes.
                    ]))

            JSONFile(
                name: "ModuleName-catalyst.symbols.json",
                content: makeSymbolGraph(
                    moduleName: "ModuleName", platform: .init(operatingSystem: .init(name: "ios"), environment: "macabi"),
                    symbols: [
                        makeSymbol(id: "first-symbol-id", kind: .class, pathComponents: ["First"]),  // No in-source availability attributes.
        ...
        #expect(
            secondRenderPlatforms.compactMap(\.name) == ["iOS", "Mac Catalyst"],
        ...
        let catalog = Folder(name: "unit-test.docc") {
            JSONFile(
                symbolGraph: makeSymbolGraph(
                    moduleName: "ModuleName", platform: .init(operatingSystem: .init(name: "macosx")),
                    symbols: [
    ...
    func symbolIsConsideredDeprecatedWithWarningWhenAvailableWithDeprecationSummaryDirective(_ deprecationSummaryLocation: DirectiveLocation, _ availabilitySource: AvailabilitySource) async throws {
        ...
        let context = try await load(catalog: catalog)
        withKnownIssue(
            "The DeprecationSummary validation doesn't know about default availability from the Info.plist",
            {
    ...
    func unconditionallyDeprecatedSymbolWithoutSpecificPlatformAvailabilityIsConsideredDeprecated() async throws {
        let catalog = Folder(name: "unit-test.docc") {
            JSONFile(
                symbolGraph: makeSymbolGraph(
                    moduleName: "ModuleName", platform: .init(operatingSystem: .init(name: "ios")),
                    symbols: [
                        makeSymbol(
                            id: "some-symbol-id", kind: .class, pathComponents: ["SomeClass"],
                            availability: [
